...
def main():
 ...
 #   lo=LiftOver("hg38", "hg19")
    ...
    All_credsets=pd.read_parquet(args.input_credset)
    #Study_SNPs=All_credsets.loc[All_credsets["study_id"].str.contains(args.Study_ID)]
...
#    Study_SNPs['hg19_pos']=Study_SNPs.apply(lambda x: Liftover_pos(input_chr=x['tag_chrom'], input_pos=x['tag_pos'], input_LO=lo), axis=1)
    ...
    Study_SNPs['names']=Study_SNPs.apply(lambda x: x['tag_chrom']+"_"+str(x['tag_pos']), axis=1)
    Study_SNPs=Study_SNPs[['names', 'tag_chrom', 'tag_pos']]

    Study_SNPs.to_csv(args.outdir+args.Study_ID+"_hg38.txt", sep="\t", index=False, header=False)

    cmd=['python3', "CHEERS_computeEnrichment.py", args.Study_ID, args.Enrichment_outdir, args.input_peak, args.outdir+args.Study_ID+"_hg38.txt"]
